...
def masked_softmax(vector: torch.Tensor,
    ...
    if mask is None:
        ...
        if not memory_efficient:
            ...
            masked_vector = vector.masked_fill( ((1 - mask.long()).byte()).bool(), mask_fill_value)
...
def replace_masked_values(tensor: torch.Tensor, mask: torch.Tensor, replace_with: float) -> torch.Tensor:
    ...
    return tensor.masked_fill(( (1 - mask.long()).byte()).bool(), replace_with)
...
def get_best_span(span_start_logits: torch.Tensor, span_end_logits: torch.Tensor) :
    ...
    span_log_mask = torch.tensor(a, device= device)
    #logger.info("span_log_mask1:{}".format(span_log_mask.shape))
    span_log_mask = span_log_mask.log()

    #logger.info("span_log_mask2:{}".format(span_log_mask.shape))
    ...
    best_scores = valid_span_log_probs.view(batch_size, -1).max(-1).values
    #valid_span_log_probs.view(batch_size, -1)
    # (batch_size, 20)
    span_start_indices = best_spans // passage_length
    span_end_indices = best_spans % passage_length

    # (batch_size, 20, 2)
    return torch.stack([span_start_indices, span_end_indices], dim=-1), best_scores
